...
let StudentsService = class StudentsService {
    ...
    async list(filters) {
        ...
        if (filters.speciality)
            qb.andWhere('speciality.id = :speciality', { speciality: Number(filters.speciality) });
        if (filters.level)
            qb.andWhere('level.id = :level', { level: Number(filters.level) });
        if (filters.section)
            qb.andWhere('section.id = :section', { section: Number(filters.section) });
        if (filters.group)
            qb.andWhere('group.id = :group', { group: Number(filters.group) });
        return qb.getMany();
    }
