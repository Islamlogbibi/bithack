...
import { mockTeacherSchedule } from '../../data/mockTeacher'

const DAYS = ['Samedi', 'Dimanche', 'Lundi', 'Mardi', 'Mercredi', 'Jeudi']
const TIME_SLOTS = ['08:00', '09:45', '11:30', '14:00', '15:45', '17:15']
...
export default function TeacherSchedule() {
  ...
  const schedule: TeacherScheduleItem[] = mockTeacherSchedule
          ...
          {TIME_SLOTS.map((time) => (
            <div key={time} className="grid grid-cols-7 border-b border-border last:border-0">
              ...
              {DAYS.map((day) => {
                ...
                return (
                  <div key={`${day}-${time}`} className="p-2 border-r border-border last:border-0 min-h-[72px]">
                    {session ? (
                      <motion.div
                      ...
                      >
                        <p className="font-bold text-sm leading-tight">{session.group}</p>
                        <p className="mt-0.5 leading-tight">{session.course}</p>
                        <p className="mt-1 opacity-80">{session.room}</p>
                  ...
                  {daySessions.map((s, i) => (
                    ...
                    >
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-xs mt-0.5 opacity-70">Groupe {s.group} · {s.room}</p>
                      </div>
